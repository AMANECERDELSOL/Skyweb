...
const Packages = () => {
  ...
  const handlePackageSelect = (packageItem: Package) => {
    const message = encodeURIComponent(
      `¡Hola Skyweb! Me interesa el "${packageItem.name}" de ${packageItem.speed} por $${packageItem.price} MXN mensuales en la zona ${currentZone.name}. Costo de instalación: $${packageItem.installationCost}. ¿Podrían darme más información?`
    );
    window.open(`https://wa.me/5219992102204?text=${message}`, '_blank');
  };

  const handleAddToCart = (packageItem: Package) => {
    addToCart({
      id: `package-${packageItem.id}`,
      type: 'package',
      name: `${packageItem.name} - ${packageItem.speed}`,
      price: packageItem.price,
      zone: currentZone.name,
      installationCost: packageItem.installationCost
    });
  };
  return (
              ...
              <div className="text-center">
                ...
                <div className="mb-4">
                  ...
                  <div className="text-sm text-gray-500 mt-1">
                    + ${packageItem.installationCost} instalación
                  </div>
                </div>
          ...
          <div className="text-center mt-16">
            <div className="bg-gradient-to-r from-cyan-500/10 to-blue-500/10 rounded-2xl p-8 border border-cyan-500/20 hover:border-cyan-500/40 transition-all duration-500 transform hover:scale-105">
              ...
              <button
                onClick={() => {
                  const message = encodeURIComponent(
                    '¡Hola Skyweb! Mi zona no aparece en la lista de paquetes. ¿Podrían verificar la disponibilidad de servicio en mi área?'
                  );
                  window.open(`https://wa.me/5219992102204?text=${message}`, '_blank');
                }}
